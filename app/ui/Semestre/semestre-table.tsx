...
export function SemesterTable({
  ...
  const [editingSubject, setEditingSubject] = useState<{ subject: Subject; semesterId: string } | null>(null)
  ...
  const handleSubmitEditSubject = async (formData: FormData) => {
    if (editingSubject) {
      // Para edición, solo actualizamos el nombre por ahora
    ...
    return (
      <Card>
        <CardContent className="text-center py-8">
          <p className="text-gray-500 mb-4">No hay semestres registrados aún.</p>
          <p className="text-sm text-gray-400">Usa el formulario de arriba para agregar el primer semestre.</p>
        </CardContent>
      </Card>
    )
  }
    ...
    <div className="space-y-4">
      {semesters.map((semester) => (
        <Card key={semester.id}>
          <Collapsible open={openSemesters.has(semester.id)} onOpenChange={() => toggleSemester(semester.id)}>
      ...
      {editingSubject && (
        <SubjectModal
          ...
          semesterName={semesters.find((s) => s.id === editingSubject.semesterId)?.name || ""}
